...
theorem Hoeffding_general {X : ℕ → Ω → ℝ}  (h_indep : iIndepFun X μ)  {n : ℕ}(hn:n>0)(h_measurable:∀i,Measurable (X i))
  ...
  have h_subG : ∀ i ∈ Finset.range n,HasSubgaussianMGF (fun ω =>X i ω - μ[X i]) (((M i - m i).toNNReal ^ 2)/4) μ :=by
    ...
    calc
      ∫⁻ (a : Ω), ‖(fun ω ↦ rexp (s * (X i ω - ∫ (x : Ω), X i x ∂μ))) a‖ₑ ∂μ =∫⁻ (a : Ω),ENNReal.ofReal ((fun ω ↦ rexp (s * (X i ω - ∫ (x : Ω), X i x ∂μ))) a )∂μ := by
        congr with a
        rw [enorm_eq_ofReal_abs]
        simp [ENNReal.ofReal]
      _ = ENNReal.ofReal (∫ (a : Ω), (fun ω ↦ rexp (s * (X i ω - ∫ (x : Ω), X i x ∂μ))) a ∂μ) := by sorry
      _ ≤ ENNReal.ofReal (rexp ((M i - m i) ^ 2 / 4 * s ^ 2 / 2)) := by
        apply ENNReal.ofReal_le_ofReal
        simp[show max (M i - m i) 0 =M i -m i by simp;exact le_of_lt (h i)] at mgf_le1
        simpa using mgf_le1 s
      _  < ⊤ := by simp
    intro t
    apply mgf_le1
  apply le_trans
